      ...
      <div class="container-fluid">
        <div class="row">
          ...
          <div class="col-md-9">
            ...
            $vista = "SELECT p.idpersona , e.idempleado, p.pnombre,p.snombre,p.pappellido,p.sapellido, c.correo, te.telefono, t.idturno, e.cargo  FROM EMPLEADO e
              INNER JOIN persona p ON p.idpersona=e.persona
              INNER JOIN turno t ON t.idturno=e.turno
              INNER JOIN telefono te ON te.persona=p.idpersona
              INNER JOIN correo c ON c.persona=p.idpersona
              WHERE e.idempleado=$id";

            $empleado = $conexion->ejecutarConsulta($vista);

            //OBTENER EL ID DE PERSONA PARA EDITAR
            $idPersona = "SELECT p.idpersona FROM EMPLEADO E 
            INNER JOIN persona p ON p.idpersona=e.persona
            WHERE e.idempleado=$id";

            $resultado = $conexion->ejecutarConsulta($idPersona);
/*
            while( $info = $conexion->obtenerFila($empleado) ){
             echo "<pre>";
             var_dump($info);
             echo "</pre>";
            }
*/
           ...
           <?php while( $info = $conexion->obtenerFila($empleado) ){ ?>
           <!-- Editar -->
           <div class="panel panel-default">
            <div class="panel-heading main-color-bg">
              <h3 class="panel-title">Editar Empleado <?php echo $id; ?> </h3> 
            </div>
            <div class="panel-body">
                <div class="row">
                  ...
                  <div class="col-xl-6 col-md-6 col-sm-12 col-12">
                    <div class="form-group">
                      <label>Cargo</label>
                      <input id="cargo" type="text" class="form-control" placeholder="" name="cargo" value="<?php echo $info["CARGO"] ?>">
                    </div>
                    <div class="form-group">
                      <label>Turno</label>

                        <?php $turno = $info["IDTURNO"]; ?> 
                            ...
                            switch ( $turno  ) {
                              case '1': ?>
                                 <select id="turno" name="turno" class="form-control">
                                    <option selected="selected"  value="1">Diurno</option>
                        ...
                        <?php break;
                              case '2': ?>
                                  <select id="turno" name="turno" class="form-control">
                                    ...
                                    <option selected="selected" value="2">Nocturno</option>
                                    <option  value="3">Mixto</option>
                                  </select>
                         <?php break;
                              case '3': ?>
                                  <select id="turno" name="turno" class="form-control">
                                    ...
                                    <option selected="selected" value="3">Nocturno</option>
                                  </select>
                          <?php break;
                            } ?>
                    </div>
                ...
                <button id="btn-guardar" class="btn btn-primary">Guardar Cambios</button>
